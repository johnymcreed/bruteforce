...
var Row = function(domain)
{
    ...
    this.set_sec = function()
    {
        if(this.sec != null)
        {
            ...
            this.element.children[1].innerHTML = 'N/A'
    }
    this.set_num = function()
    {
        if(this.num != null)
        {
            ...
            this.element.children[2].innerHTML = 'N/A'
    }
    this.set_username = function()
    {
        if(this.username)
            ...
            this.element.children[3].innerHTML = 'N/A'
    }
    ...
    this.set_actions = function()
    {
        if( this.actions.inputs )
        {
            if( ! this.element.children[5].children.length || this.element.children[5].children[0].getAttribute('action') != 'select_inputs' )
                this.element.children[5].innerHTML = '<i class="fa-solid fa-arrow-pointer" action="select_inputs" host="' + this.domain + '"></i>'
        }
        else
            this.element.children[5].innerHTML = 'N/A'
        ...
        if( this.actions.wordlist )
        {
            if( ! this.element.children[6].children.length || this.element.children[6].children[0].getAttribute('action') != 'load_wordlist' )
                this.element.children[6].innerHTML = '<i class="fa-solid fa-spell-check" action="load_wordlist" host="' + this.domain + '"></i>'
        }
        else
            this.element.children[6].innerHTML = 'N/A'
        ...
        if( this.actions.start )
        {
            if( ! this.element.children[7].children.length || this.element.children[7].children[0].getAttribute('action') != 'start_brute' )
                this.element.children[7].innerHTML = '<i class="fa-solid fa-play" action="start_brute" host="' + this.domain + '"></i>'
        }
        else if( this.actions.pause )
        {
            if( ! this.element.children[7].children.length || this.element.children[7].children[0].getAttribute('action') != 'pause_brute' )
                this.element.children[7].innerHTML = '<i class="fa-solid fa-pause" action="pause_brute" host="' + this.domain + '"></i>'
        }
        else
            this.element.children[7].innerHTML = 'N/A'

        if( this.actions.delete )
        {
            if( ! this.element.children[8].children.length || this.element.children[8].children[0].getAttribute('action') != 'delete_brute' )
                this.element.children[8].innerHTML = '<i class="fa-solid fa-trash-xmark" action="delete_brute" host="' + this.domain + '"></i>'
        }
        else if( this.actions.create )
        {
            if( ! this.element.children[8].children.length || this.element.children[8].children[0].getAttribute('action') != 'create_target' )
                this.element.children[8].innerHTML = '<i class="fa-solid fa-plus" action="create_target"></i>'
        }
        else
            this.element.children[8].innerHTML = 'N/A'
    }
    ...
    this.update_handlers = function()
    {
        ...
        for(var i = 0; i < items.length; i++)
            ...
            {
                items[i].addEventListener( 'click', (function(callback,host) { return function() {
                    callback(host)
                } } )( actions[ items[i].getAttribute('action') ], items[i].getAttribute('host') ) )

                if( items[i].getAttribute('action') == 'select_inputs' )
                    items[i].addEventListener( 'mouseover', (function(callback,host) { return function() {
                    callback(host)
                } } )( actions.highlight_inputs, items[i].getAttribute('host') ) )
