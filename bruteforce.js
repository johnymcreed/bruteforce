...
var DOM_helper = {
    ...
    search_element: function (where, what) {
        for (var i = 0; i < where.children.length; i++) {
            var elem = where.children[i],
                is_found = true,
                matches = 0;
            for (var attr in what) {
                if (elem.getAttribute(attr) == what[attr]) matches++;
                else is_found = false;
            }
            if (is_found && matches) break;

            if (matches)
                this.similar_elements.push({ elem: elem, matches: matches });
            if ((elem = this.search_element(elem, what))) return elem;
        }
        if (is_found) return elem;
    },
...
var Brute = function () {
    var user_field_element,
        password_field_element,
        submit_field_element,
    ...
    this.highlight_inputs = function () {
        ...
        highlight_field_element(_user_field_element, "rgba(255, 255, 255, 0.44)");
        highlight_field_element(_password_field_element, "rgba(255, 255, 255, 0.44)");
        highlight_field_element(_submit_field_element, "rgba(255, 255, 255, 0.44)");
    ...
    this.setting = function () {
        if (!this.is_setting_done) return;
        ...
        window.alert(
            "Follow and click each senquence:\n" +
                "1. Username Input\n" +
                "2. Password Input\n" +
                "3. Submit/Login/Go/Ect Button\n\n" +
			"Warnings and Information:\n" +
				"A. If a login form does not have these 3 items above then you cannot use this extension here.\n" +
				"B. If for some reason what you clicked didn't highlight, know that it still is selected.\n" +
				"C. If it didn't work and what was selected didn't function correctly please send an issue to the github page.\n\n" +
			
			"Have fun!"
        );

        var on_click_handler = (function (that) {
            return function (e) {
                //console.log('click')
                if (!that.user_field) {
                    //console.log('user_field')
                    that.user_field = DOM_helper.element_to_object(e.target);
                    highlight_field_element(e.target, "rgba(255, 255, 255, 0.44)");
                } else if (!that.password_field) {
                    //console.log('password_field')
                    that.password_field = DOM_helper.element_to_object(
                        e.target
                    );
                    highlight_field_element(e.target, "rgba(255, 255, 255, 0.44)");
                    ...
                    document
                        ...
                        .removeEventListener("click", on_click_handler);
                }
            };
        })(this);
        ...
        document
            .getElementsByTagName("body")[0]
            .addEventListener("click", on_click_handler);
                        ...
                        if (
                            ...
                            bg_send_message({
                                ...
                                data: {
                                    host: location.host,
                                    founded: {
                                        user: previously_user,
                                        password: previously_password,
                                    },
                                },
                            });
                            bg_send_message({
    ...
    this.login = function (
        ...
        if (user_field_element) user_field_element.value = this.creds.user;
...
if (in_frame()) throw bruteforce_exception("skipping frame");
