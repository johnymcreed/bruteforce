...
var select_inputs = function (host) {
    ...
    if ((target = targets.find(host))) {
        ...
        if ((tab = target.find(tabs.get_current_tab())))
            content_page_send_message(tab.id, {
                method: "select_inputs",
                data: "",
            });
        else
            content_page_send_message(target.tabs[0].id, {
                method: "select_inputs",
                data: "",
            });
    }
    ...
    if (target)
        chrome.windows.create({
            url: chrome.runtime.getURL(
                "../wordlist/wordlist.html#" + target.tabs[0].id
            ),
            type: "popup",
            width: 600,
            height: 300,
...
var start_brute = function (host) {
    ...
    for (var i = 0; i < target.tabs.length; i++)
        content_page_send_message(target.tabs[i].id, {
            method: "start_brute",
            data: "",
        });
};
...
window.onload = function () {
    ...
    console.log(window.location.pathname, VERSION);
};
