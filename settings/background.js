...
var Target = function (tab) {
	...
	this.remove = function (tab) {
		for (var i = 0; i < this.tabs.length; i++)
			if (this.tabs[i].id == tab.id) this.tabs.splice(i, 1);
	};
	this.find = function (tab) {
		if (tab)
			for (var i = 0; i < this.tabs.length; i++)
				if (this.tabs[i].id == tab.id) return this.tabs[i];
	};
};
...
var Tabs = function () {
	...
	this.push = function (tab) {
		var old_tab;
		if ((old_tab = this.get(tab.id))) this.remove(old_tab);
		this.tabs.push(tab);
	};
	this.find = function (host) {
		for (var i = 0; i < this.tabs.length; i++)
			if (this.tabs[i].host == host) return this.tabs[i];
	};
	this.get_current_tab = function () {
		for (var i = 0; i < this.tabs.length; i++)
			if (this.tabs[i].id == current_tab_id) return this.tabs[i];
	};
	this.forEach = function (callback) {
		for (var i = 0; i < this.tabs.length; i++) callback(this.tabs[i], i, this.tabs);
	};
};
...
var Targets = function () {
	...
	this.get = function (id) {
		for (var i = 0; i < this.targets.length; i++)
			for (var j = 0; j < this.targets[i].tabs.length; j++)
				if (this.targets[i].tabs[j].id == id) return this.targets[i];
	};
	this.find = function (host) {
		for (var i = 0; i < this.targets.length; i++)
			if (this.targets[i].host == host) return this.targets[i];
	};
	...
	this.get_current_target = function () {
		for (var i = 0; i < this.targets.length; i++)
			for (var j = 0; j < this.targets[i].tabs.length; j++)
				if (this.targets[i].tabs[j].id == current_tab_id) return this.targets[i];
	};
	this.forEach = function (callback) {
		for (var i = 0; i < this.targets.length; i++) callback(this.targets[i], i, this.targets);
	};
};
...
var Dictionary = function (wordlist) {
	...
	this.get_wordlist = function (id) {
		for (var dict = 0; dict < wordlists.length; dict++)
			if (wordlists[dict].id == id) return wordlists[dict];
	};

	this.del_wordlist = function (id) {
		for (var dict = 0; dict < wordlists.length; dict++)
			if (wordlists[dict].id == id) wordlists.splice(dict, 1);
	};

	this.pop_creds = function () {
		var creds = this.get_current_creds();
		if (creds.user != null && creds.password != null) this.offset++;
		return creds;
	};
	...
	this.get_count = function () {
		var count = 0;
		for (var dict = 0; dict < wordlists.length; dict++) {
			for (var i = 0; i < wordlists[dict].combo.length; i++) count++;

			if (wordlists[dict].users && wordlists[dict].passwords)
				for (var i = 0; i < wordlists[dict].passwords.length; i++)
					for (var j = 0; j < wordlists[dict].users.length; j++) count++;
		}
		return count;
	};
...
var content_handlers = {
	page_ready: function (tabid, data) {
		...
		if (!(tab = tabs.get(tabid))) {
			...
			if ((target = targets.find(tab.host))) target.remove(tab);
			tab.host = data.host;
		}
	...
	put_founded: function (tabid, data) {
		var target;
		if ((target = window.targets.find(data.host)) != null) target.founded.push(data.founded);
	},
	wait_target: function (tabid) {
		var target = targets.get(tabid);
		if (target) return { settings: target.settings, status: target.status };
	},
...
var select_inputs = function (host) {
	...
	if ((target = targets.find(host))) {
		...
		else content_page_send_message(target.tabs[0].id, { method: 'select_inputs', data: '' });
	}
};
...
var highlight_inputs = function (host) {
	...
	if ((target = targets.find(host))) {
		...
		else content_page_send_message(target.tabs[0].id, { method: 'highlight_inputs', data: '' });
	}
};
...
var open_dict_window = function (host) {
	...
	if (target)
		chrome.windows.create({
			url: chrome.runtime.getURL('wordlist/wordlist.html#' + target.tabs[0].id),
			type: 'popup',
			width: 600,
			height: 300,
		});
...
var set_interval = function (host, interval) {
	...
	if (!isNaN(interval)) target.settings.brute_interval = interval;
};
...
var set_offset = function (host, offset) {
	...
	if (!isNaN(offset)) target.settings.dictionary.offset = offset;
};
...
function content_page_recv_message(obj, tabid) {
	...
	if (obj) return content_handlers[obj.method](tabid, obj.data);
}
